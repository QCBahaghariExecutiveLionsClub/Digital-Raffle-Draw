 ...
 *
 * NOTE: as of Sept 2026, this intentionally no longer requires a
 * verification code — anyone with the ticket link can rename it, to make
 * sharing/reassigning tickets frictionless for older, non-technical users.
 * Every change is still timestamped and logged to the Notes column (below)
 * for an audit trail, since there's no other guard against a wrong or
 * malicious rename.
 */
